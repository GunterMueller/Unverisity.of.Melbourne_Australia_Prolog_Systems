...
static	Word	*par_matches;	/* holds segment matches for each key */
static	Word	*slice;		/* buffer to read bit-sliced seg descriptors */
static	Int	slice_size;	/* how many bits in seg match bit-strings */
static	Word	*index_block;	/* block of seg descs for DSIMC index scheme */
...
#if 0
fprintf(stderr, "open_seg need:%d sop:%d fd:%x fop:%d\n",
		need_to_read,sd->sd_operation,fd->fd_file,fd->fd_opn);
#endif
	if (!need_to_read && iswriting(sd->sd_operation) &&
		fd->fd_file != FileNULL && !iswriting(fd->fd_opn)) {
		close(fd->fd_file);
...
Segt *sd;
{
	...
	else if (iswriting(sd->sd_operation)) {
