...
static Word	*par_matches;	/* holds record matches for each key */
static Word	*slice;		/* buffer to read bit-sliced rec descriptors */
static Int	slice_size;	/* how many bits in rec match bit-strings */
...
fprintf(stderr,"inserting %s\n",tr->input);
	...
	if (rec_offset < seg_base(seg)+seg_size(rel)) {
		...
		nchars = write(datafile(seg), (Char *)seg_buf, seg_size(rel));
		if (nchars != seg_size(rel))
			return(-1);
	}
	else {
		lseek(datafile(seg), seg_base(seg), L_SET);
		nchars = write(datafile(seg), (Char *)seg_buf, seg_info_size(rel));
		if (nchars != seg_info_size(rel))
			return(-1);
		lseek(datafile(seg), (Faddr)rec_offset, L_SET);
		nchars = write(datafile(seg), start_keys, rec_size);
		if (nchars != rec_size)
			return(-1);
	}
...
fprintf(stderr,"%s -> (%d,%d) [%08x]\n",
	...
	nchars = write(datafile(seg), (Char *)seg_buf, seg_size(rel));
	if (nchars != seg_size(rel)) {
		fprintf(stderr, "Can't write data file for insertion\n");
		return(-1);
	}
...
Trans *tr;
{
	 ...
	 */
	lseek(datafile(seg), seg_base(seg), L_SET);
	nchars = write(datafile(seg), (char *)seg_buffer(seg), seg_info_size(rel));
	if (nchars != seg_info_size(rel)) {
		fprintf(stderr, "Can't write data file for deletion\n");
		return(-1);
	}
	...
	when SIMC_INDEX:
		...
		if (counters[tr->cur_seg] > Nr(rel))
			...
			counters[tr->cur_seg]--;
		lseek(descfile(rel), coffset(rel), L_SET);
		nchars = write(descfile(rel), (char *)counters, ncounters);
		free((char *)counters);
		if (nchars != ncounters) {
			fprintf(stderr, "Can't update counters for deletion\n");
			return(-1);
		}
	when DSIMC_INDEX:
		Nlr(rel,tr->cur_level)--;
...
fprintf(stderr,"off:%d sz:%d nx:%d fspace:%x eos:%x\n",
	...
	seg_ptr->sp_freespace, end_of_seg);
		...
		else if (rec_ptr[r].rp_offset != UNUSED &&
...
Trans *tr;
{
	...
	free((char *)counters);
	if (nchars != ncounters)
		fprintf(stderr, "Can't write counters for insertion\n");
	return(-1);
}
...
Trans *tr;
{
	...
	for (i = 0, bit = tr->seg_list; i < tr->nsegbits; i++, bit++)
	{
		...
		if (!b_test(seg_no, slice, Ns(rel))) {
			...
			lseek(descfile(rel), slice_offset, L_SET);
			nchars = write(descfile(rel), (char *)slice, nBytes(slice_size));
			if (nchars != nBytes(slice_size))
				fprintf(stderr, "Can't update segment descriptors\n");
		}
	}
#ifdef DBUG
	debug(ADD_DBUG) putc('\n',stderr);
...
#endif /* DBUG */
	lseek(descfile(rel), block_offset, L_SET);
	nchars = write(descfile(rel), (char *)index_block, block_size(rel));
	free((char *)index_block);
	if (nchars != block_size(rel))
		fprintf(stderr, "Can't update segment descriptors\n");
}
