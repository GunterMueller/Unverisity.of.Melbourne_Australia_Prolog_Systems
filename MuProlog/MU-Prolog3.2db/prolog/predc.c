...
#ifdef NEWDBASE
    {
      ...
      if (IsComp(t1) && ctdict(t1) == D_dbab) {
        ...
        */
        ...
        if (!IsFunc(t2))
          fprintf(stderr, "db_abort predicate not there!\n");
        else
...
#define boundto_(r, l)                                                         \
  (IsVar(*(Ptr *)*r) && vtdict(*(Ptr *)*r) == D_ && *(levtype *)*(r + 2) == l)
...
static bound_(r, l, cs) /* does reset(r) refer to a binding of _ in the term */
    /* cs at level l ? */
    Int *r;
levtype l;
Ptr cs;
...
#define boundtolocal(r, t, l)                                                  \
  (occurs((Ptr) * (Ptr) * r, (levtype) * (Ptr) * (r + 2), t, l) == SUCCEED)
...
levtype l;
{
  ...
  if (!unify(t2, l2, t3, l3)) {
    ...
    for (r1 = r; r1 < rsetend; r1 = r1 + 4)
      if (!boundlocal((levtype)*r1, t1, l1) && !boundtolocal(r1, t1, l1))
        return (DELAY);
    reset(grset(gtop));
    rsetend = grset(gtop); /* needed? */
...
levtype l;
{
  ...
  bufp = &savebuf.l[S_CONSTS];
  if (strcmp(savebuf.header, SAVEHDR) != 0 || *bufp++ != (Int)VERSION ||
      *bufp++ != (Int)GSTKLEN || *bufp++ != (Int)DICTLEN ||
      *bufp != (Int)onintr || !restdata(fd)
#ifdef DLOAD
      || !restdload(fd)
#endif
      || read(fd, (char *)&t, 1) != 0) {
