...
/*
rlhp.c
functions to access external database using recursive linear hashing
*/
...
p_rlhp_query(t, l)		/* Prolog fn to initiate query from external DB */
	Ptr t;
	levtype l;
{
	Ptr tq, td;
...
p_rlhp_end(t, l)			/* called at end of db query */
	Ptr t;
	levtype l;
{
...
p_rlhp_abort(t, l)			/* called to abort db query */
	Ptr t;
	levtype l;
{
	findbind((Ptr)targ(1, t), l, &t, &l);
...
p_rlhp_retract(t, l)	/* Prolog fn to retract in external DB */
	Ptr t;
	levtype l;
{
	Ptr td, r;
...
p_rlhp_rall(t, l)	/* Prolog fn to retract all in external DB */
	Ptr t;
	levtype l;
{
	Ptr td, r;
...
p_rlhp_assert(t, l)	/* Prolog fn to assert in external DB */
	Ptr t;
	levtype l;
{
	Ptr r, td;
...
static mudd_req(dbname,t, l,type)
...
static dbq_new(dbname, pred_int, slot)
	char *dbname;
	int pred_int;
	int slot;
{
...
static dbq_write(slot, t, l, type)		/* write query down pipe */
	int slot;
	Ptr t;
	levtype l;
	char type;
...
static mudd_clear() /* not used currently ??? */
{
	register i;
